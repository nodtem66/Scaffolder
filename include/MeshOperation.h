...
inline bool fix_non_manifold_edges(TMesh& mesh, double minimum_diameter, uint16_t max_iteration = 5, bool verbose = false) {
    size_t nf = 1;
    ...
    for (uint16_t iteration = 0; (edgeBorderNum > 0 || edgeNonManifoldNum > 0) && iteration < max_iteration; iteration++) {
        if (verbose) std::cout << "[Fix non-manifold edges]" << std::endl << "  -- Iteration " << iteration + 1 << std::endl;
        ...
        if (edgeNonManifoldNum > 0) {
            ...
            if (vcg::tri::Clean<TMesh>::CountNonManifoldEdgeFF(mesh) > 0) {
                std::cout << "[Warning]: closing holes failed: Mesh has some not 2-manifold edges" << std::endl;
            }
            else {
...
inline bool fix_non_manifold_vertices(TMesh& mesh, double minimum_diameter, uint16_t max_iteration = 5, bool verbose = false) {
    vcg::tri::UpdateTopology<TMesh>::FaceFace(mesh);
    ...
    for (uint16_t iteration = 0; (vertManifNum > 0) && iteration < max_iteration; iteration++) {
        if (verbose) std::cout << "[Fix non-manifold vertices]" << std::endl << "  -- Iteration " << iteration + 1 << std::endl;
        if (verbose) std::cout << "  -- non-manifold vertices: " << vertManifNum << std::endl;
