...
namespace optimal_slice
{
    ...
    public:
        ...
        Triangle() : v{Point3d{0, 0, 0}, Point3d{0, 0, 0}, Point3d{0, 0, 0}}, min{0.0, 0.0, 0.0}, max{0.0, 0.0, 0.0} {}
    ...
    {
        double angle;
        if (DOUBLE_EQ(p.x, line.x))
            angle = 90;
        else if (DOUBLE_EQ(p.y, line.y))
            angle = 0;
        else
            angle = atan((p.y - line.y) / (p.x - line.x)) * 180 / M_PI;
        // calculate delta
        if (DOUBLE_GT_EQ(line.theta, angle))
            angle = line.theta - angle;
        else
            angle = 180 + line.theta - angle;
        // normalized angle
        if (angle > 90)
            angle -= 180;
        return angle;
    ...
    {
        double angle;
        if (DOUBLE_EQ(delta.x, 0))
            angle = 90;
        else if (DOUBLE_EQ(delta.y, 0))
            angle = 0;
        else
    ...
    public:
        ...
        FeretDiameter(const SupportLine2d (&lines)[4])
        {
            ...
            {
                min = d2;
                angleMin = lines[1].theta;
                max = d1;
                angleMax = lines[0].theta;
            }
            else
            {
                min = d1;
                angleMin = lines[0].theta;
                max = d2;
                angleMax = lines[1].theta;
            }
            perimeter = 0;
...
{
    template <>
    struct hash<optimal_slice::Point2d>
    {
...
namespace optimal_slice
{
    ...
    inline void build_triangle_list(TMesh &mesh, size_t grid_size, Plane &P, Layer &L, int direction = Direction::Z)
    {
        ...
        std::vector<Layer> local_L(num_threads, Layer(grid_size + 2));

#pragma omp parallel
        {
            int thread_id = omp_get_thread_num();
            auto &_L = local_L[thread_id];

#pragma omp for nowait
            for (int i = 0; i < static_cast<int>(mesh.face.size()); i++)
            {
                if (!mesh.face[i].IsD())
                {
        ...
        // Calculate the total number of triangles per level across all threads.
        std::vector<size_t> level_sizes(grid_size + 2, 0);
        std::vector<std::vector<size_t>> offsets(grid_size + 2, std::vector<size_t>(num_threads + 1, 0));
        for (size_t level = 0; level <= grid_size + 1; ++level)
        {
            size_t offset = 0;
            for (int thread_id = 0; thread_id < num_threads; ++thread_id)
            {
                offsets[level][thread_id] = offset;
                offset += local_L[thread_id][level].size();
                offsets[level][thread_id + 1] = offset;
            }
            level_sizes[level] = offset;
        }

        // Resize the final buffers once and fill them from disjoint ranges.
        for (size_t level = 0; level <= grid_size + 1; ++level)
        {
            L[level].clear();
            L[level].resize(level_sizes[level]);
        }

#pragma omp parallel for schedule(static)
        for (int level = 0; level <= grid_size + 1; ++level)
        {
            auto &out = L[level];
            for (int thread_id = 0; thread_id < num_threads; ++thread_id)
            {
                auto &src = local_L[thread_id][level];
                if (!src.empty())
                {
                    const auto start = offsets[level][thread_id];
...
#pragma omp for schedule(static)
                ...
                {
                    const Triangle &triangle = A[idx];
                    if (triangle.max[direction] < P[i])
                    {
                        continue;
                    }

                    if (triangle.max[direction] >= P[i] && triangle.min[direction] <= P[i])
                    {
                        Line line;
                        if (compute_intersection(triangle, P[i], line, direction))
                        {
#pragma omp critical
                            {
                                S[i - 1].push_back(line);
...
#pragma omp parallel for schedule(static)
    ...
    PolygonSides contour_inside_test(const Polygons &C, std::vector<std::pair<size_t, size_t>> &pairList)
    {
        ...
        for (size_t i = 0, size = C.size(); i < size; i++)
        {
            ...
            if (countInside[i] == 0)
                position[i] = PolygonSide::OUTSIDE;
    ...
    double gjk_minimal_distance(const Polygon &P, const Polygon &Q, double tolerance = 1e-2)
    {
        ...
        while (_count > 0)
        {
            if (DOUBLE_EQ(d.x * d.x + d.y * d.y, 0))
                return 0;
...
namespace optimal_slice
{
    ...
    bool write_svg(std::string filename, const Polygons &C, const float width, const float height, const int min_x, const int min_y, bool show_convexhull = false)
    {
        ...
        for (Polygons::const_iterator t = C.begin(); t != C.end(); t++)
        {
            ...
            if (p[index] == PolygonSide::INSIDE)
                color = "f00";
            else if (p[index] == PolygonSide::OUTSIDE)
                color = "000";
            else if (p[index] == PolygonSide::PAIRED)
                color = "00f";
...
#else
#pragma omp parallel for schedule(static)
        for (int cs_index = 0; cs_index < static_cast<int>(CS.size()); cs_index++)
        {
#endif
            if (CS[cs_index].empty())
                continue;
