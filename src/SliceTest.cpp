...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        options.add_options()("h,help", "Print help")("i,input", "Input file (STL,PLY,OFF,OBJ)", cxxopts::value<std::string>(), "FILE")("s,slice", "Number of slice > 1 (Default: 10)", cxxopts::value<std::size_t>(), "POSITVE INTEGER MORE THAN 1")("d,direction", "Direction 0=X, 1=Y, 2=Z default: 2)", cxxopts::value<int>(), "{0,1,2}")("x,convexhull", "Export convexhull to SVG", cxxopts::value<bool>());
        ...
        if (result.count("slice"))
            slice = result["slice"].as<std::size_t>();
        if (slice < 2)
        {
            std::cout << "Invalid slice: " << slice << std::endl;
        }
        if (result.count("direction"))
            direction = result["direction"].as<int>();
...

    {
        try
        {
            ...
            {
                if (verbose)
                    std::cout << "Import " << format << ": " << input_file << std::endl;
                int loadmark = 0;
                if (format == "stl")
                {
...
#pragma omp parallel for schedule(static)
                for (int index = 0; index < static_cast<int>(C.size()); ++index)
                {
                    std::stringstream name(dir);
                    name << dir << "/" << index << ".svg";
