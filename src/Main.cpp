...
#define LOG                                      \
    if (log_format == SCAFFOLDER_FORMAT_DEFAULT) \
    log
#define CSV                                  \
    if (log_format == SCAFFOLDER_FORMAT_CSV) \
    log
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (isEmptyOption || result.count("help"))
        {
            std::cout << options.help() << std::endl
                      ...
                      << "    " << "Generated BCC scaffold with w=3.14159 (PI), t=0, and grid size=100" << std::endl
                      << std::endl
                      ...
                      << "    " << "grid size=100, k_slice=100, k_polygon=4" << std::endl
                      << std::endl
                      << "  " << util::PathGetBasename(argv[0]) << " input.stl output.stl bcc,3.14159,0 -m -q --format csv" << std::endl
                      << "    " << "Generated and evaluated BCC scaffold (w=3.14159, t=0) and reported in CSV" << std::endl
                      << std::endl
                      << std::endl
                      << "Lua file: " << std::endl
                      << std::endl
                      ...
                      << "  -----------------------------------------------------------------" << std::endl
                      << std::endl
                      << "Special symbols can be used in lua file: " << std::endl
                      << std::endl
        ...
        if (result.count("dirty"))
            dirty = result["dirty"].as<bool>();
        if (result.count("no_intersect"))
            is_intersect = !result["no_intersect"].as<bool>();
        ...
        if (result.count("params"))
        {
            ...
            if (n >= 1)
                surface_name = params[0];
            if (n >= 2)
                coff = std::stod(params[1]);
            if (n >= 3)
                isolevel = std::stod(params[2]);
            if (n >= 4)
                minimum_grid_size = std::stoi(params[3]);
            if (n >= 5)
                k_slice = std::stoi(params[4]);
            if (n >= 6)
                k_polygon = std::stoi(params[5]);
        }
        if (result.count("isolevel"))
            isolevel = result["isolevel"].as<double>();
        if (result.count("grid_size"))
            minimum_grid_size = result["grid_size"].as<size_t>();
        if (result.count("grid_offset"))
            grid_offset = result["grid_offset"].as<uint16_t>();
        if (result.count("coff"))
            coff = result["coff"].as<double>();
        if (result.count("minimum_diameter"))
            minimum_diameter = result["minimum_diameter"].as<double>();
        if (result.count("surface"))
            surface_name = result["surface"].as<std::string>();
        if (result.count("shell"))
            shell = result["shell"].as<uint16_t>();
        if (result.count("smooth_step"))
            smooth_step = result["smooth_step"].as<uint16_t>();
        if (result.count("fix_self_intersect"))
            fix_self_intersect = result["fix_self_intersect"].as<int>();
        if (result.count("size_optimize"))
            qsim_percent = result["size_optimize"].as<double>();
        if (result.count("k_slice"))
            k_slice = result["k_slice"].as<uint16_t>();
        if (result.count("k_polygon"))
            k_polygon = result["k_polygon"].as<uint16_t>();
    ...
    else CSV << "Surface,coff,shell,thickness,grid_size,grid_offset,smooth_step,input_file,";
    ...
    try
    {
        ...
        if (!dirty)
        {
            ...
            if (fix_self_intersect > 0)
                fix_self_intersect_mesh(mesh, minimum_diameter, fix_self_intersect, verbose_stream);
            if (is_build_inverse)
                clean_mesh(inverse_mesh, minimum_diameter, 0, verbose_stream);
        }
    ...
    try
    {
        if (is_analysis_microstructure && is_manifold)
        {
            ...
            if (minFeret.size() > 0 && maxFeret.size() > 0)
            {
                ...
                else CSV << std::accumulate(minFeret.begin(), minFeret.end(), 0.0) / minFeret.size() << ','
    ...
    try
    {
        if (is_mean_curvature)
        {
            ...
            else
            {
                ...
                else CSV << H.Percentile(0.1f) << "," << H.Percentile(0.25f) << "," << H.Percentile(0.5f) << "," << H.Percentile(0.75f) << "," << H.Percentile(0.9f) << ",";
            }
    ...
    // Stage 2
    {
        ...
        if (!pointcloud && watertight)
        {
            ...
            else CSV << abs(volume2) << ',' << area2 << ',' << 1 - abs(volume2 / volume1) << ',' << area2 / area1 << ','
        ...
        else
        {
            LOG << "[Warning] The scaffold isn't a manifold, so the suggestions are here:\n"
                ...
                << std::endl;
            else CSV << ",,,," << mesh.VN() << ',' << mesh.FN() << std::endl;
        }
        ...
        if (!no_output)
        {
            ...
            else if (output_format == "stl")
            {
                ...
                if (is_build_inverse)
                    vcg::tri::io::ExporterSTL<TMesh>::Save(inverse_mesh, filename2.c_str(), true, 0);
            }
            else if (output_format == "ctm")
            {
                vcg::tri::io::ExporterCTM<TMesh>::Save(mesh, filename.c_str(), 0, false, 1e-3);
                if (is_build_inverse)
                    vcg::tri::io::ExporterCTM<TMesh>::Save(inverse_mesh, filename2.c_str(), 0, false, 1e-3);
            }
            LOG << "OK" << std::endl;
        }
